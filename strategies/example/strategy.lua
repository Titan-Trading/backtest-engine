...
function on_bar(bar)
    ...
    if rsi < rsi_buy_threshold and not get_position(symbol, exchange) then
        execute_market_order(symbol, order_quantity, "buy")
    end

    -- If RSI is above a given threshold and we have a position, sell
    if rsi > rsi_sell_threshold and get_position(symbol, exchange) then
        execute_market_order(symbol, order_quantity, "sell")
    end
