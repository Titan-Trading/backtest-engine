...
impl ThreadPool {
        ...
        F: FnOnce() + Send + 'static,
    {
        ...
        if let Err(error) = self.sender.send(Message::NewJob(job)) {
            println!("was not able to send job to worker: {:?}", error);
        }
    }
...
// represents the different types of tasks that can be performed using the thread pool
enum TaskType {
    FileWrite,
    FileRead,
    RecordSynchronizationAndConsolidation,
}

// represents that a struct can process tasks using the thread pool
pub trait Task {
